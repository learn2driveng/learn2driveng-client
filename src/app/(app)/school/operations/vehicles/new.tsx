...
export default function NewSchoolVehicleScreen() {
  ...
  const [photo, setPhoto] = useState<{ uri: string; fileName: string; mimeType: string | null; size: number | null } | null>(null);
  ...
  const choosePhoto = async (source: "camera" | "library") => {
    const result = source === "camera"
      ? await (async () => {
          ...
          if (!permission.granted) throw new Error("Camera access is required to take a vehicle photograph.");
          return ImagePicker.launchCameraAsync({ mediaTypes: ["images"], allowsEditing: true, aspect: [4, 3], quality: 0.8 });
        })()
      : await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images"], allowsEditing: true, aspect: [4, 3], quality: 0.8 });
    if (result.canceled) return;
    const asset = result.assets[0];
    setPhoto({ uri: asset.uri, fileName: asset.fileName ?? "vehicle.jpg", mimeType: asset.mimeType ?? null, size: asset.fileSize ?? null });
  };
  ...
  return (
    <DashboardScreen>
      ...
      >
        <View>
          <Text className="mb-2 text-[11px] uppercase tracking-[1.4px]" style={{ color: colors.textSubtle, fontFamily: fontFamily.figtreeBold }}>Vehicle photograph</Text>
          <View className="flex-row items-center gap-4 rounded-2xl" style={{ backgroundColor: colors.surfaceStrong }}>
            <View className="h-20 w-28 items-center justify-center overflow-hidden rounded-2xl" style={{ backgroundColor: colors.surfaceMuted }}>
              {photo ? <Image source={{ uri: photo.uri }} className="h-full w-full" resizeMode="cover" /> : <MaterialCommunityIcons name="car-hatchback" size={32} color={colors.textSubtle} />}
            </View>
            <View className="flex-1 gap-2 pr-3">
              <Pressable onPress={() => void choosePhoto("library")} className="h-9 items-center justify-center rounded-xl" style={{ backgroundColor: colors.primary }}><Text className="text-[11px]" style={{ color: colors.onPrimary, fontFamily: fontFamily.figtreeBold }}>{photo ? "Change photograph" : "Choose photograph"}</Text></Pressable>
              <Pressable onPress={() => void choosePhoto("camera")} className="h-9 flex-row items-center justify-center gap-1.5 rounded-xl" style={{ backgroundColor: colors.surface }}><MaterialCommunityIcons name="camera-outline" size={16} color={colors.text} /><Text className="text-[11px]" style={{ color: colors.text, fontFamily: fontFamily.figtreeBold }}>Take photo</Text></Pressable>
            </View>
          </View>
        </View>
        {[
          ...
          <View key={field.label}>
            ...
            <TextInput
              ...
              autoCapitalize={field.label === "Plate number" ? "characters" : "words"}
              onChangeText={(value) =>
                field.onChangeText(

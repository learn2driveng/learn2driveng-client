...
export default function SchoolLessonDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        (caught as ApiError).message || "We could not load this lesson.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const cancel = () => {
    ...
    Alert.alert(
      "Cancel lesson",
      "Existing learner bookings will be cancelled for this lesson.",
      [
            ...
            try {
              ...
              setError(
                (caught as ApiError).message ||
                  "We could not cancel this lesson.",
              );
            } finally {
              setCancelling(false);
            }
  ...
  if (loading)
    return (
      <DashboardScreen>
        <DashboardPageHeader title="Lesson" />
      ...
      <DashboardScreen>
        ...
        <View className="mt-8">
          <ContentEmptyState
            ...
            description={error ?? "This lesson is no longer available."}
          />
        </View>
      </DashboardScreen>
    );
  ...
  const lessonPackages = packages.filter((item) =>
    session.eligiblePackageIds.includes(item.id),
  );
  const seatsLeft = Math.max(0, session.capacity - session.participantCount);
  const fillPercentage = session.capacity
    ? Math.min(100, (session.participantCount / session.capacity) * 100)
    : 0;
  ...
  return (
    <DashboardScreen>
      ...
      <View
        className="mt-7 p-5"
        style={{ backgroundColor: colors.contrastSurface, borderRadius: 28 }}
      >
        ...
        >
          <View
            ...
            style={{
              ...
              backgroundColor: seatsLeft ? colors.primary : colors.error,
            }}
          />
        </View>
        <View className="mt-2 flex-row justify-between">
          ...
          >
            {session.participantCount} booked
          </Text>
          <Text
            className="text-[10px]"
            style={{
      ...
      <View className="mt-8">
        ...
        <Pressable
          ...
          accessibilityLabel="View assigned instructor"
          accessibilityState={{ disabled: !instructor }}
          disabled={!instructor}
          onPress={() =>
            instructor &&
            router.push({
        ...
        >
          <InstructorAvatar
            ...
            initials={instructor?.initials ?? "IN"}
            photoUrl={instructor?.profilePhoto}
          />
          <View className="flex-1">
            <Text
              className="text-[15px]"
        ...
        >
          ...
          >
            ...
            <Text
              ...
              style={{ color: colors.text, fontFamily: fontFamily.figtreeBold }}
            >
              {lessonPackages.map((item) => item.name).join(" · ") ||
                "Selected packages"}
            </Text>
          </View>
        </View>

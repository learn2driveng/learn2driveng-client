...
export default function PurchaseReviewScreen() {
  ...
  const { schoolId, packageId, method = "card" } = useLocalSearchParams<{
    schoolId?: string;
    packageId?: string;
    method?: string;
  }>();
  ...
  return (
    <CheckoutShell title="Review purchase" step={2} onBack={() => router.back()}>
      <ScrollView
        className="flex-1"
      ...
      >
        ...
        >
          ...
          <Text
            ...
            style={{ color: colors.contrastMuted, fontFamily: fontFamily.figtree }}
          >
        ...
        >
          ...
          >
            By paying, you agree to the school’s cancellation and rescheduling policy.
          </Text>
        </View>
      </ScrollView>
      ...
      >
        <Pressable
          ...
          onPress={() =>
            router.replace({
              pathname: "/student/explore/[schoolId]/checkout/result",
        ...
        >
          <MaterialCommunityIcons name="lock-outline" size={19} color={colors.onPrimary} />
          <Text
            className="text-[15px]"
